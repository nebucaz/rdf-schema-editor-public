<script lang="ts">
	...
	function handleSelectionChange({ nodes: selNodes, edges: selEdges }: { nodes: Node[]; edges: Edge[] }) {
		selectedIri = selNodes[0]?.id ?? selEdges[0]?.id ?? null;
	}
	...
	function nextPosition() {
		...
		return { x: (i % 4) * 260, y: Math.floor(i / 4) * 220 };
	}
	...
	let deleteEntityWarning = $state<string[] | null>(null);
	...
	async function handleCreateEntity(name: string, description: string, color: string | undefined) {
		const { iri } = await sparqlConnector.insertClass(name, description || undefined);
		if (color) nodeColorStore.setColor(iri, color);
		const newNode: EntityNodeType = {
			id: iri,
			type: 'entity',
			position: nextPosition(),
			data: makeNodeData(iri, name, description, [], color)
		};
		nodes = [...nodes, newNode];
		showAddEntity = false;
	}
	...
	async function handleEditEntitySubmit(name: string, description: string, color: string | undefined) {
		...
		if (name !== node.data.name) {
			await sparqlConnector.renameClass(node.data.classIri, name);
		}
		if (description !== node.data.description) {
			await sparqlConnector.updateClassDescription(node.data.classIri, description || null);
		}
		nodeColorStore.setColor(node.data.classIri, color);
		updateNodeData(editEntityId, (d) => makeNodeData(d.classIri, name, description, d.attributes, color, d.members));
		editEntityId = null;
	}
	...
	async function handleDeleteEntityConfirm(force: boolean) {
		...
		errorMessage = null;
		try {
			const result = await sparqlConnector.deleteClass(classIriValue, { force });
			if (!result.deleted) {
				deleteEntityWarning = result.externalReferences;
				return;
			}
	...
	async function handleAttributeSubmit(
		...
		if (editingAttribute) {
			await sparqlConnector.updateDatatypeProperty(node.data.classIri, editingAttribute.iri, {
				name,
				datatype,
				required,
				repeatable
	...
	async function handleDeleteAttributeConfirm() {
		...
		errorMessage = null;
		try {
			await sparqlConnector.deleteDatatypeProperty(attribute.iri, classIriValue);
	...
	async function handleAddMember(label: string) {
		if (!manageMembersClassIri) return;
		const { iri } = await sparqlConnector.insertIndividual(manageMembersClassIri, label);
	...
	async function handleEditMember(member: EntityMemberVM, label: string) {
		if (!manageMembersClassIri) return;
		await sparqlConnector.renameIndividual(member.iri, label);
	...
	async function handleDeleteMember(member: EntityMemberVM) {
		if (!manageMembersClassIri) return;
		await sparqlConnector.deleteIndividual(member.iri);
		updateNodeData(manageMembersClassIri, (d) => ({
			...d,
	...
	async function createInheritanceEdge(source: string, target: string) {
		errorMessage = null;
		try {
			const result = await sparqlConnector.insertSubClassOf(source, target);
	...
	async function handleDeleteInheritanceConfirm() {
		...
		errorMessage = null;
		try {
			await sparqlConnector.deleteSubClassOf(edge.source, edge.target);
	...
	async function handleCreateAssociation(name: string, description: string, links: AssociationLink[]) {
		const result = await sparqlConnector.insertAssociationClass(name, description || undefined, links);
		const newNode: EntityNodeType = {
			id: result.iri,
			type: 'entity',
			position: nextPosition(),
			data: makeNodeData(result.iri, name, description, [], nodeColorStore.getColor(result.iri))
		};
	...
	async function handleEditAssociationSubmit(
		...
		if (name !== node.data.name) {
			await sparqlConnector.renameClass(classIriValue, name);
		}
		if (description !== node.data.description) {
			await sparqlConnector.updateClassDescription(classIriValue, description || null);
		}
		...
		updateNodeData(classIriValue, (d) => makeNodeData(d.classIri, name, description, d.attributes, color, d.members));
	...
	async function handleRemoveExternalStub(nodeId: string) {
		...
		try {
			...
			for (const e of touching) {
				await sparqlConnector.deleteSubClassOf(e.source, e.target);
			}
	...
	async function loadSchemaFromGraphDB() {
		...
		try {
			...
			const schema = await sparqlConnector.fetchFullSchema();
	...
	async function handleToggleAssociation(classIriValue: string, isAssociation: boolean) {
		errorMessage = null;
		try {
			await sparqlConnector.setAssociationClass(classIriValue, isAssociation);
...
<div class="editor">
	<div class="toolbar">
		...
		<button
			class="add-entity secondary-action"
			onclick={() => void loadSchemaFromGraphDB()}
			disabled={loading}
		>
			{loading ? 'Loading…' : '⟳ Reload from GraphDB'}
		</button>
		<button class="add-entity secondary-action" onclick={() => (showTriplesPanel = !showTriplesPanel)}>
			{showTriplesPanel ? 'Hide Triples' : 'View Triples'}
		</button>
		{#if errorMessage}
...
<Modal isOpen={showAddEntity} title="Add Entity" onClose={() => (showAddEntity = false)}>
	<EntityForm submitLabel="Create" onCancel={() => (showAddEntity = false)} onSubmit={handleCreateEntity} />
</Modal>
...
<Modal isOpen={editEntityId !== null} title="Edit Entity" onClose={() => (editEntityId = null)}>
	{#if editingNode}
		<label class="association-toggle">
			<input
				type="checkbox"
				checked={lastAssociationClassIris.has(editingNode.data.classIri)}
				onchange={(e) => void handleToggleAssociation(editingNode.data.classIri, e.currentTarget.checked)}
			/>
			Treat as attributed-relationship (association) class
		</label>
		<p class="hint">
			Writes or removes this class's `rdfs:subClassOf AttributedRelationship` marker triple in
			GraphDB (STORY-020), then reloads the canvas — drawing this class's outgoing relations as
			attributed-relationship links (diamond marker) instead of plain relation edges, or vice versa.
		</p>
		<EntityForm
...
>
	{#if deletingNode}
		{#if deleteEntityWarning && deleteEntityWarning.length > 0}
			<p>
			...
			<ul class="warning-list">
				{#each deleteEntityWarning as ref (ref)}
...
<Modal isOpen={showAddAssociation} title="Add Attributed Relationship" onClose={() => (showAddAssociation = false)}>
	<AssociationForm {entityOptions} onCancel={() => (showAddAssociation = false)} onSubmit={handleCreateAssociation} />
</Modal>
...
<Modal isOpen={showAddExternalClass} title="Add External Class Reference" onClose={() => (showAddExternalClass = false)}>
	<ExternalClassForm onCancel={() => (showAddExternalClass = false)} onSubmit={handleAddExternalClass} />
</Modal>
