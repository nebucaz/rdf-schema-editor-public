<script lang="ts">
	...
	export interface AssociationEditLinkRow {
		/** Present for a link that already exists on this class; absent for a link added in this
		 *  editing session (not yet persisted). Used by the caller to diff against the class's
		 *  current links on submit — see `handleEditAssociationSubmit` in `+page.svelte`. */
		iri?: string;
		propName: string;
	...
	interface Props {
		...
		initialColor?: string;
		initialLinks: AssociationEditLinkRow[];
	...
	let { initialName, initialDescription, initialColor, initialLinks, entityOptions, onSubmit, onCancel }: Props =
		$props();
	...
	let links = $state<AssociationEditLinkRow[]>(initialLinks.map((l) => ({ ...l })));
...
<form onsubmit={handleSubmit}>
	...
	<div class="links-section">
		...
		{#each links as link, i (link.iri ?? `new-${i}`)}
			<div class="link-row">
	...
	<div class="actions">
		...
		<button type="submit" class="primary" disabled={submitting}>{submitting ? 'Saving…' : 'Save'}</button>
	</div>
</form>
